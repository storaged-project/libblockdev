...
build-introspection-data: build-library ${LIBRARY_FILES}
	g-ir-scanner `pkg-config --cflags --libs glib-2.0` --library=blockdev -I src/lib/ -L src/lib/ --identifier-prefix=BD --symbol-prefix=bd --namespace BlockDev --nsversion=1.0 -o BlockDev-1.0.gir --warn-all src/lib/blockdev.h src/lib/blockdev.c src/lib/plugins.h src/lib/plugin_apis/lvm.h
...
test-from-python: build-introspection-data
	GI_TYPELIB_PATH=. LD_LIBRARY_PATH=src/plugins/ python -c 'from gi.repository import BlockDev; BlockDev.init(None); print BlockDev.lvm_get_max_lv_size()'
